...
class MyChecker(checkerlib.BaseChecker):
    ...
    def check_service(self):
        # check if ports are open
        if not self._check_port_web(self.ip, PORT_WEB) or not self._check_port_ssh(self.ip, PORT_MYSQL):
            return checkerlib.CheckResult.DOWN
        #else
        # check if server is Apache 2.4.50
        ...
        # check if dev1 user exists in pasapasa_ssh docker
        ##MUAif not self._check_ssh_user('dev1'):
        ##MUA    return checkerlib.CheckResult.FAULTY
        ...
        ##MUA    return checkerlib.CheckResult.FAULTY            
        ##MUAreturn checkerlib.CheckResult.OK
